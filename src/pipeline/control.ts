...
export type AiRecord = { provider: string; model: string; resolvedModel?: string; retryAt?: string; retryScope?: "model"; promptVersion: string; stage: "scoring" | "analysis";
...
export interface ControlStore {
  ...
  providerRetryAt?(provider: "groq" | "openrouter", now: string, model?: string): Promise<string | null>;
...
function makeStore(access: <T>(fn: (state: State) => T, write: boolean) => Promise<T>): ControlStore {
  return {
    providerRetryAt: (provider, now, model) => access((s) => s.usage
      .filter((row) => row.ai?.provider === provider && row.ai.retryAt && row.ai.retryAt > now &&
        (row.ai.retryScope !== "model" || (model !== undefined && row.ai.model === model)))
